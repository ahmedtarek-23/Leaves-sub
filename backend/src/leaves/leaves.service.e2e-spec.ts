...
describe('LeavesService E2E Tests', () => {
    ...
    describe('Scenario 1: Standard Submission and Approval (BR 31)', () => {
        it('should complete full approval workflow with sufficient balance', async () => {
            ...
            leaveEntitlementModel.findOne.mockResolvedValue(initialEntitlement);
            leaveTypeModel.findOne.mockResolvedValue({ _id: leaveTypeId, name: 'Annual Leave' });
            leavePolicyModel.findOne.mockResolvedValue({
                _id: new Types.ObjectId(),
                leaveTypeId: leaveTypeId,
                payrollPayCode: 'ANNUAL_LEAVE',
            });
            calendarModel.find.mockResolvedValue([]); // No holidays
            leaveRequestModel.find.mockResolvedValue([]); // No overlapping leaves
            ...
            leaveRequestModel.findOne.mockResolvedValue(null); // No existing requests
            leaveRequestModel.find.mockResolvedValue([]); // No team conflicts
            ...
            leaveEntitlementModel.find.mockResolvedValue([afterSubmissionEntitlement]);
            ...
            leaveRequestModel.findById.mockResolvedValue(mockRequest);
            leaveRequestModel.findByIdAndUpdate.mockResolvedValue(requestAfterManagerReview);
            ...
            leaveRequestModel.findById.mockResolvedValue(requestAfterManagerReview);
            leaveRequestModel.findByIdAndUpdate.mockResolvedValue(requestAfterHRReview);

            // Mock finalizeIntegration
            leaveEntitlementModel.findOneAndUpdate.mockResolvedValue(
            ...
            leaveEntitlementModel.find.mockResolvedValue([finalEntitlement]);
    ...
    describe('Scenario 2: Unpaid Leave Conversion (BR 29)', () => {
        it('should convert excess days to unpaid leave when balance is insufficient', async () => {
            ...
            calendarModel.find.mockResolvedValue([]);
            leaveRequestModel.find.mockResolvedValue([]);
            ...
            leaveEntitlementModel.findOneAndUpdate.mockResolvedValue(updatedEntitlement);
            leavePolicyModel.findOne.mockResolvedValue({
                _id: new Types.ObjectId(),
                leaveTypeId: leaveTypeId,
                payrollPayCode: 'ANNUAL_LEAVE',
    ...
    describe('Scenario 3: Administrative Adjustments and Audit (BR 17)', () => {
        it('should create audit trail for manual adjustments', async () => {
            ...
            leaveEntitlementModel.find.mockResolvedValue([updatedEntitlement]);
    ...
    describe('Scenario 4: Encashment Limit (BR 53)', () => {
        it('should enforce 30-day cap on leave encashment', async () => {
            ...
            leaveTypeModel.findOne.mockResolvedValue(annualLeaveType);
            ...
            leaveRequestModel.findById.mockResolvedValue(approvedRequest);
            ...
            leaveEntitlementModel.findOne
                .mockResolvedValueOnce(entitlementForEncash) // First call in encashLeave
                .mockResolvedValueOnce(entitlementAfterEncash); // For balance check
