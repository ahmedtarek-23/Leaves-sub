...
export class PerformanceService {
  ...
  async updateTemplate(id: string, updates: Partial<CreateTemplateDto>) {
    ...
    if (!template) throw new NotFoundException('Template not found');
    if (updates.name) {
      const other = await this.templateModel.findOne({
        name: updates.name,
        _id: { $ne: id },
      });
      if (other)
        throw new ConflictException(
          `Template name "${updates.name}" already in use`,
        );
    }
    ...
    if (usedInActive)
      throw new ConflictException(
        'Cannot modify template used in an active cycle',
      );
  ...
  async deactivateTemplate(id: string) {
    ...
    if (!template.isActive)
      throw new ConflictException('Template already deactivated');
    const usedInActive = await this.cycleModel.findOne({
      'templateAssignments.templateId': id,
      status: AppraisalCycleStatus.ACTIVE,
    });
    if (usedInActive)
      throw new ConflictException(
        'Cannot deactivate template used in an active cycle',
      );
  ...
  async createCycle(dto: CreateCycleDto) {
    for (const ta of dto.templateAssignments) {
      ...
      if (!template)
        throw new BadRequestException(`Template ${ta.templateId} not found`);
      for (const depId of ta.departmentIds) {
        const dep = await this.departmentModel.findById(depId);
        if (!dep)
          throw new BadRequestException(`Department ${depId} not found`);
      }
    }
    ...
    for (const ta of dto.templateAssignments) {
      for (const depId of ta.departmentIds) {
        ...
        if (overlapping)
          throw new ConflictException(
            `Overlapping cycle for department ${depId}`,
          );
      }
    }
  ...
  async activateCycle(id: string) {
    ...
    if (cycle.status !== AppraisalCycleStatus.PLANNED)
      throw new ConflictException('Only planned cycles can be activated');
    if (!cycle.templateAssignments?.length)
    ...
    for (const a of dto.assignments) {
      ...
      if (!emp)
        throw new BadRequestException(
          `Employee ${a.employeeProfileId} not found`,
        );
      if (!dep)
        throw new BadRequestException(`Department ${a.departmentId} not found`);
      if (a.managerProfileId) {
        const mgr = await this.employeeModel.findById(a.managerProfileId);
        if (!mgr)
          throw new BadRequestException(
            `Manager ${a.managerProfileId} not found`,
          );
      }
      if (a.positionId) {
        const pos = await this.positionModel.findById(a.positionId);
        if (!pos)
          throw new BadRequestException(`Position ${a.positionId} not found`);
      }
      ...
      if (!template)
        throw new BadRequestException(`Template ${a.templateId} not found`);
      if (
        Array.isArray((template as any).applicableDepartmentIds) &&
        (template as any).applicableDepartmentIds.length
      ) {
        const tplDeps = (template as any).applicableDepartmentIds.map(
          (d: any) =>
            Types.ObjectId.isValid(d)
              ? new Types.ObjectId(d).toString()
              : d.toString(),
        );
      ...
      if (
        Array.isArray((template as any).applicablePositionIds) &&
        (template as any).applicablePositionIds.length &&
        a.positionId
      ) {
        const tplPos = (template as any).applicablePositionIds.map((p: any) =>
          Types.ObjectId.isValid(p)
            ? new Types.ObjectId(p).toString()
            : p.toString(),
        );
      ...
      if (existing)
        throw new ConflictException(
          'Assignment already exists for employee in cycle',
        );
      prepared.push({
        ...a,
  ...
  async getAssignmentsForManager(managerId: string, user?: AuthUser) {
    if (
      user &&
      !['HR_MANAGER', 'HR_ADMIN', 'SYSTEM_ADMIN'].includes(user.role)
    ) {
  ...
  async getAssignmentById(id: string) {
    ...
    const assignment = await this.assignmentModel
      .findById(id)
      .populate(
        'cycleId templateId employeeProfileId managerProfileId departmentId positionId',
      )
      .exec();
  ...
  async submitRecord(dto: SubmitRecordDto, user?: AuthUser) {
    if (
      ...
      if (user.employeeId !== (dto.managerProfileId as unknown as string))
    ...
    if (
      ...
      throw new ConflictException(
        'Cannot submit record for an assignment not in progress',
      );
    }
    if (!dto.ratings?.length)
      throw new BadRequestException('Ratings must be provided');
    ...
    for (const r of dto.ratings) {
      if (r.ratingValue == null && r.weightedScore == null)
        throw new BadRequestException(
          'Each rating must contain ratingValue or weightedScore',
        );
      ...
      if (ratingScale) {
        ...
        if (min != null && value < min)
          throw new BadRequestException(`Rating ${value} below minimum ${min}`);
        if (max != null && value > max)
          throw new BadRequestException(`Rating ${value} above maximum ${max}`);
      }
      totalScore += value;
    }
  ...
  async publishRecord(id: string, hrPublishedById?: string) {
    ...
    if (record.status !== AppraisalRecordStatus.MANAGER_SUBMITTED)
      throw new ConflictException(
        'Only manager-submitted records can be published',
      );
    if (record.hrPublishedAt)
      throw new ConflictException('Record already published');
    const cycle = await this.cycleModel.findById(record.cycleId);
    if (cycle?.status === AppraisalCycleStatus.CLOSED)
      throw new ConflictException('Cannot publish for a closed cycle');
    if (hrPublishedById) {
      const hrUser = await this.employeeModel.findById(
        new Types.ObjectId(hrPublishedById),
      );
      if (!hrUser)
        throw new BadRequestException(
          'hrPublishedById is not a valid employee',
        );
    }
  ...
  async acknowledgeRecord(
    ...
    if (record.status !== AppraisalRecordStatus.HR_PUBLISHED)
      throw new ConflictException(
        'Cannot acknowledge a record not published by HR',
      );
    const empObjId =
      typeof employeeId === 'string'
        ? new Types.ObjectId(employeeId)
        : employeeId;
    if (record.employeeProfileId.toString() !== empObjId.toString())
      throw new BadRequestException(
        'Employee cannot acknowledge another employee record',
      );
    ...
    if (activeDispute)
      throw new ConflictException(
        'Cannot acknowledge while active dispute exists',
      );
  ...
  async raiseDispute(dto: RaiseDisputeDto, user?: AuthUser) {
    if (
      ...
      if (user.employeeId !== (dto.raisedByEmployeeId as unknown as string))
    ...
    if (record.employeeAcknowledgedAt)
      throw new ConflictException(
        'Cannot raise dispute after employee has acknowledged the record',
      );
    ...
    if (daysSincePublish > 7)
      throw new BadRequestException(
        'Dispute window has closed. Appeals must be filed within 7 days of publication.',
      );
  ...
  async getDisputeById(id: string) {
    const dispute = await this.disputeModel
      .findById(id)
      .populate(
        'appraisalId assignmentId cycleId raisedByEmployeeId resolvedByEmployeeId',
      );
    if (!dispute) throw new NotFoundException('Dispute not found');
    return dispute;
  }
  ...
  async getEmployeeAppraisalTrends(employeeId: string, user?: AuthUser) {
    ...
    const history = await this.getEmployeeAppraisalHistory(
      employeeId,
      undefined,
      user,
    );
    ...
    return {
      ...
      trends,
      averageScore: trends.length
        ? trends.reduce((s, t) => s + (t.totalScore || 0), 0) / trends.length
        : 0,
    };
  }
  ...
  async generateCycleReport(cycleId: string) {
    ...
    const stats = {
      ...
      completedRecords: records.filter(
        (r) => r.status === AppraisalRecordStatus.HR_PUBLISHED,
      ).length,
      ...
      openDisputes: disputes.filter(
        (d) => d.status === AppraisalDisputeStatus.OPEN,
      ).length,
    };
  ...
  async generateDepartmentReport(departmentId: string, cycleId?: string) {
    ...
    const averageScore = records.length
      ? records.reduce((sum, r) => sum + (r.totalScore || 0), 0) /
        records.length
      : 0;
    return {
  ...
  async dashboardStats(departmentId?: string) {
    ...
    return {
      ...
      completionRate: totalAssignments
        ? (publishedRecords / totalAssignments) * 100
        : 0,
    };
  }
  ...
  async getDepartmentProgress(cycleId?: string) {
    ...
    for (const assignment of assignments) {
      ...
      dept.completionRate =
        dept.total > 0
          ? ((dept.published + dept.acknowledged) / dept.total) * 100
          : 0;
    }
